...
Compile stages L0-L2 and the initial run-build path are active. Deferred stages remain explicit no-ops where noted.
...
| `scr_tune_compile.gml` | `run_build`, `run_build_stages` + `run_build_stage_*` | Compiled events become scheduler events with pointing, pulse and late ornament expansion using the established runtime catalog. Numeric/lead anchors emit component UIDs tied to host events; catalog replacement, trailing anchors and piobaireachd validation are deferred. | compiled tune, run config, `datafiles/embellishments.json` | returned playback events | `scr_button_prepare_single_tune_playback_events` |
...
| `scr_tune_authoring.gml` | `tune_author_validate_dependencies` | Import acceptance gate: aggregate unknown ornament definitions, validate selected rhythm profiles, and warn when a meter has no straight pulse default. Errors reject the tune before any artifacts are written. | compiled tune, libraries, existing manifest | diagnostics | `tune_author_create_from_abc` |
| `scr_embellishments.gml` | `load_embellishment_library` | Load the established runtime ornament catalog and merge the small validated additions catalog. The paused replacement CSV is never loaded. | `embellishments.json`, `embellishment_additions.json` | returned library | `obj_game_controller` Create |
