...
| `scr_game_viz.gml` | `gv_measure_nav_hit_test(_mx, _my)` | Hit-tests tune-structure panel and now returns full tile identity (`measure`, `part`, `nav_idx`) for part-aware selection and popup routing. | `global.timeline_state.measure_nav_controls`, `global.timeline_state.measure_nav_tile_hitboxes` | none | `gv_measure_nav_handle_click`, `gv_review_handle_click` |
...
| `scr_game_viz.gml` | `gv_draw_tune_structure_panel(_x1, _y1, _x2, _y2)` | Tune-structure panel now supports display-row policy for canonical model metadata: pickup rows (`display_row_kind=pickup_row`) are hidden by default via `timeline_cfg.tune_structure_show_pickup_rows=false` for compatibility. Current-tile highlight and auto-follow prefer source-nav identity resolved from normalized panel time (`nav_display_ms`) with measure fallback, reducing loop/repeated-measure drift. | `global.timeline_cfg.tune_structure_show_pickup_rows`, `global.timeline_state.measure_nav_entries`, canonical model helpers | tile hitboxes/controls state | RoomUI tune-structure anchor draw path |
...
| `scr_game_viz.gml` | `gv_resolve_measure_context(_time_ms)` | Central structural resolver: prefers canonical tune-structure model windows when enabled, then mapper output, then legacy measure fallback, normalizing reads (`measure`, `part`, `nav_idx`, `measure_ref_key`) in one place. | `gv_tune_structure_model_resolve_context_at_time`, `map_time_to_context`, `gv_get_current_planned_measure`, highlight cache globals | none | `gv_sync_now_line_display`, tune-structure panel draw, world overlay draw, segment-transition reseed, score-lane probe |
...
| `scr_game_viz.gml` | `gv_draw_tune_structure_panel(_x1, _y1, _x2, _y2)` | Draws tune-structure grid and drives playback auto-follow scroll from local measure-nav entry time windows (single source of truth); for single-tune loop sessions (live and post-play), projects the display to a canonical one-pass structure instead of loop-expanded runtime rows, normalizing loop playhead/review time back onto first-pass timing so highlight/follow stay readable; emits `page_turn` log entries when follow changes rows and writes tile hitboxes with part-aware selection identity (`part:measure`). | `global.timeline_state.*`, `global.playback_context`, `global.timeline_cfg` | `global.timeline_state.measure_nav_scroll_row`, `global.timeline_state.measure_nav_total_rows`, `global.timeline_state.measure_nav_view_rows`, `global.timeline_state.measure_nav_tile_hitboxes`, `global.timeline_state.measure_nav_controls`, `global.timeline_state.measure_nav_auto_follow_last_ms` | gameviz panel draw pipeline |
...
| `scr_game_viz.gml` | `gv_perf_summary_get_latest(_force_refresh)` | Loads and caches the latest JSONL run summary from `performances/run_summaries.jsonl` for status tile and detail panel rendering. | `scr_data_paths_get_category_root` (optional), local static cache | none | `gv_draw_gameviz_controls_panel`, `gv_handle_gameviz_controls_click`, `gv_draw_notebeam_scoring_panel` |
...
| `scr_game_viz.gml` | `gv_perf_summary_build_lines(_summary)` | Builds detailed metric lines for the phase-1 performance popup (mode/title, elapsed, c95/c99/max, s95/s99/max, draw p95, midi p95, spikes). | summary struct fields | none | `gv_gameviz_draw_perf_summary_popup` |
...
| `scr_game_viz.gml` | `gv_gameviz_draw_perf_summary_popup(_x1, _y1, _x2, _y2, _summary, _replace_panel)` | Draws an on-canvas perf details popup (with close button); supports panel-replacement mode so perf details occupy the notebeam judge panel region. | `global.timeline_state.perf_summary_popup`, summary struct fields | `global.timeline_state.perf_summary_popup` | `gv_draw_notebeam_scoring_panel` |
