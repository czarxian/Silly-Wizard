...
| `scr_tune_library.gml` | `scr_data_paths_load_primary_root_from_config()` | Loads optional primary data root override from JSON config (`runtime_paths.json`) and normalizes it for runtime use (backward compatible with legacy tune-root key). | `datafiles/config/runtime_paths.json`, `datafiles/runtime_paths.json`, `runtime_paths.json` | none | `obj_game_controller` Create |
| `scr_tune_library.gml` | `scr_data_paths_save_primary_root_to_config(_root)` | Persists primary data root override changes to runtime config JSON mirrors used by IDE/runtime path lookup. | none | `datafiles/config/runtime_paths.json`, `datafiles/runtime_paths.json`, `runtime_paths.json` | settings UI handlers (`scr_settings_data_root_set`, `scr_settings_data_root_reset_auto`) |
| `scr_tune_library.gml` | `scr_tune_library_get_runtime_root()` | Resolves the tune-library root as `<primary_data_root>/tunes/` for startup/rebuild/load flows. | `global.primary_data_root_override` | none | `scr_load_tune_library`, `obj_game_controller` Create, `scr_regenerate_tune_library` |
...
| `scr_button_scripts.gml` | `scr_settings_data_root_set(_ctx)` | Opens a file picker to capture a custom primary data root, validates `<root>/tunes`, persists config JSON, regenerates library, and refreshes settings field display. | `global.primary_data_root_override` | `global.primary_data_root_override`, `global.tune_library` | button index 38 (`scr_handle_button_click`) |
| `scr_button_scripts.gml` | `scr_settings_data_root_reset_auto(_ctx)` | Clears custom primary data root override, persists AUTO config, regenerates library, and refreshes settings field display. | `global.primary_data_root_override` | `global.primary_data_root_override`, `global.tune_library` | button index 39 (`scr_handle_button_click`) |
...
| `global.primary_data_root_override` | string | Optional absolute/relative primary data root override (`.../datafiles/` style); loaded from runtime paths JSON at startup; blank = auto-detect candidate roots | `obj_game_controller` Create | `scr_data_paths_get_primary_root`, category path helpers, startup/rebuild flows |
